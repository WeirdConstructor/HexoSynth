...
pub fn setup_node_id_module() -> wlambda::SymbolTable {
    ...
    st.fun(
        ...
        move |_env: &mut Env, _argc: usize| {
            ...
            for cat in [
                ...
                let v = VVal::vec();
                cat.get_node_ids(0, |nid| {
                    v.push(node_id2vv(nid));
                });
    ...
    st.fun(
        "instance",
        move |env: &mut Env, _argc: usize| Ok(VVal::Int(vv2node_id(&env.arg(0)).instance() as i64)),
        Some(1),
        Some(1),
        false,
    );
    ...
    st.fun(
        "name",
        move |env: &mut Env, _argc: usize| Ok(VVal::new_str(vv2node_id(&env.arg(0)).name())),
        Some(1),
        Some(1),
        false,
    );
    ...
    st.fun(
        "label",
        move |env: &mut Env, _argc: usize| Ok(VVal::new_str(vv2node_id(&env.arg(0)).label())),
        Some(1),
        Some(1),
        false,
    );

    let mut info_map: std::collections::HashMap<String, VVal> = std::collections::HashMap::new();

    for nid in hexodsp::dsp::ALL_NODE_IDS.iter() {
        info_map.insert(nid.name().to_string(), VVal::new_usr(VValNodeInfo::new(*nid)));
    }
    ...
    st.fun(
        "eq_variant",
        move |env: &mut Env, _argc: usize| {
            Ok(VVal::Bol(vv2node_id(&env.arg(0)).eq_variant(&vv2node_id(&env.arg(1)))))
        },
        Some(2),
        Some(2),
        false,
    ...
    st.fun(
        ...
        move |env: &mut Env, _argc: usize| {
            ...
            Ok(VVal::map2("atoms", atoms, "inputs", inputs))
        },
        Some(1),
        Some(1),
        false,
    ...
    st.fun(
        ...
        move |env: &mut Env, _argc: usize| {
            ...
            while let Some(name) = nid.out_name_by_idx(i) {
                outputs.push(VVal::pair(VVal::Int(i as i64), VVal::new_str(name)));
                i += 1;
            }
